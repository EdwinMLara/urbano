<?php
	...
	if(isset($_GET["num"])){
		...
		if(count($datos)>0){
			...
			$pdf->Cell(15, 8, utf8_decode('Fecha de expedición de la licencia '. date('d-m-Y', strtotime($oldvigencia1))), 0,0, 'L');
			//
			$pdf->Ln(10);
			$pdf->SetFont( 'Arial', 'B', 10 );
			...
			$pdf->newFlowingBlock( 200, 5, '', 'L' );
			$pdf->SetFont( 'Arial', '', 10 );
			$pdf->WriteFlowingBlock( utf8_decode('El suscrito solicita le sea concedido el alineamiento y número oficial que correspondiera al predio que ostenta la siguiente información'));
			$pdf->finishFlowingBlock();
			$pdf->Ln(3);
			$pdf->newFlowingBlock( 200, 5, '', 'L' );
			$pdf->SetFont( 'Arial', '', 10 );
			$pdf->WriteFlowingBlock( utf8_decode('Dirección: '));
			$pdf->SetFont( 'Arial', '', 10 );
			$idvv=($datos->ubicacion_obra);
			$datosvialidada=VialidadData::getById($idvv);
			$pdf->WriteFlowingBlock( utf8_decode($datosvialidada->nombre));
			$pdf->WriteFlowingBlock( utf8_decode(' '));
			$idcc=($datos->colonia);
			$datoscolonia=ColoniaData::getById($idcc);
			$pdf->WriteFlowingBlock( utf8_decode($datoscolonia->nombre));
			/*$pdf->WriteFlowingBlock( utf8_decode($datos->colonia));*/
			$pdf->finishFlowingBlock();
			$pdf->newFlowingBlock( 200, 5, '', 'L' );
			$pdf->SetFont( 'Arial', '', 10 );
			...
			$pdf->newFlowingBlock( 200, 5, '', 'L' );
			$pdf->SetFont( 'Arial', '', 10 );
			$pdf->WriteFlowingBlock( utf8_decode('Los datos especificados a continuación, se registran bajo el entendimiento de que el frente de inmuebles está situado tal y como se muestra en el croquis; además de sér éste el registrado en el departamento de Catastro Municipal.'));
			$pdf->finishFlowingBlock();
			$pdf->Ln(5);
			$pdf->newFlowingBlock( 120, 5, '', 'L' );
			$pdf->SetFont( 'Arial', '', 10 );
			...
			if($_GET["posi"]==2){
				$pdf->Image('../plugins/imagenes/croquiseste.jpg',40,180);
				$pdf->SetXY(95,232);
			...
			elseif($_GET["posi"]==3){
				$pdf->Image('../plugins/imagenes/croquisoeste.jpg',40,180);
				$pdf->SetXY(95,232);
				$pdf->SetFont( 'Arial', '', 12 );
				$idv=($datos->ubicacion_obra);
				$datosvialidad=VialidadData::getById($idv);
			...
			*/
			...
			$ruta="../storage/numeracion/";
			$veriarchi=$datos->numero_licencia;
			$ext=".pdf";
			$archiext = $veriarchi.''.$ext;
			if(file_exists($ruta.''.$archiext)){
				unlink($ruta.''.$archiext);
