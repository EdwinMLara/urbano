<?php
	...
	$pdf->Cell(80, 8, 'Nombre Solicitante', 1,0,'C', True);
	$pdf->Cell(35, 8, 'Cuenta Catastral',1, 0,'C', True);
	$pdf->Cell(80, 8, 'Ubicacion de la Obra',1, 0,'C', True);
	$pdf->Cell(35, 8, 'Vigencia Inicial',1, 0,'C', True);
	$pdf->Cell(35, 8, 'Vigencia Final',1, 0,'C', True);
	$pdf->Ln(8);
	...
	if(isset($_GET["sd"]) && isset($_GET["ed"])){
		...
		if(count($operations)>0){
			...
			foreach($operations as $operation){
				...
				$pdf->Cell(80, 8, utf8_decode($operation->nombre_solicitante), 1,0,'C');
				$pdf->Cell(35, 8, utf8_decode($operation->predial_obra), 1,0,'C');
				$pdf->Cell(80, 8, utf8_decode($operation->ubicacion_obra), 1,0,'C');
				$oldvigencia1=$operation->vigencia1;
				$oldvigencia2=$operation->vigencia2;
				$pdf->Cell(35, 8, date('d-m-Y', strtotime($oldvigencia1)), 1,0,'C');
				$pdf->Cell(35, 8, date('d-m-Y', strtotime($oldvigencia2)), 1,0,'C');
				$pdf->Ln(8);
