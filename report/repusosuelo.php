<?php
	...
	$pdf->Cell(70, 8, 'Solicitante',1, 0,'C', True);
	$pdf->Cell(70, 8, 'Domicilio Uso Suelo',1, 0,'C', True);
	$pdf->Cell(25, 8, 'Numero',1, 0,'C', True);
	$pdf->Cell(60, 8, 'Colonia',1, 0,'C', True);
	$pdf->Ln(8);
	$pdf->SetFont('Arial', '', 9);
	if(isset($_GET["sd"]) && isset($_GET["ed"])){
		$operations = array();
		$operations = SueloData::getAllByDateOp($_GET["sd"],$_GET["ed"]);
		$totalfolio = 0;
		if(count($operations)>0){
			$supertotal = 0;
			foreach($operations as $operation){
				$pdf->Cell(15, 8, $operation->id, 1,0,'C');
				$oldfecha=$operation->fecha_alta;
				$pdf->Cell(30, 8, date('d-m-Y', strtotime($oldfecha)), 1,0,'C');
				$pdf->Cell(70, 8, utf8_decode($operation->nombre_solicitante), 1,0,'C');
				$idv= $operation->domicilio_predio;
				$datosvialidad=VialidadData::getById($idv);
				$pdf->Cell(70, 8, utf8_decode($datosvialidad->nombre), 1,0,'C');
				$pdf->Cell(25, 8, utf8_decode($operation->numero_predio), 1,0,'C');
				$idc=$operation->colonia_predio;
				$datoscolonia=ColoniaData::getById($idc);
				$pdf->Cell(60, 8, utf8_decode($datoscolonia->nombre), 1,0,'C');
				$pdf->Ln(8);
